...
signin_button = tk.Button(main_window, text="Sign In", command=login_user)
...
signup_username_label =tk.Label(main_window, text="Username:")
...
def show_signup_page(event):
    ...
    #remove sign in things
    main_signin_label.pack_forget()
    main_username_label.pack_forget()
    main_username_entry.pack_forget()
    main_password_label.pack_forget()
    main_password_entry.pack_forget()
    signin_button.pack_forget()
    signup_link.pack_forget()

    #display sign up things
    signup_label.pack()
...
signin_link.bind("<Button-1>", show_signup_page)
